...
export function useLog(){const [m,setM]=useState(loadLog);const patch=(k,o)=>setM(prev=>{const next={...prev,[k]:{...(prev[k]||{}),...o}};saveLog(next);return next});return {m,upd:patch}}
...
export const apiBase = import.meta.env.PROD ? '' : 'http://localhost:4000';
    ...
    <div className="min-h-screen bg-cream lg:h-screen lg:overflow-hidden">
      <div className="mx-auto flex min-h-screen max-w-[1700px] flex-col lg:h-screen lg:flex-row">
        ...
        <div className="min-h-0 flex-1 lg:h-full">
          <LoginForm onAuth={onAuth} apiBase={apiBase} />
        </div>
      </div>
    </div>
  );
};
function App(){const defaults={name:'',age:'',gender:'',height:'',weight:'',targetWeight:'',activity:'',health:'Normal',healthDetails:'',allergies:'',avoid:'',goal:'loss',diet:'',mealsPerDay:'',cuisine:'',plan:'monthly'};const [page,setPage]=useState('overview'),[dark,setDark]=useState(false),[route,setRoute]=useState(()=>location.hash.startsWith('#admin')?'admin':'app');useEffect(()=>{const f=()=>setRoute(location.hash.startsWith('#admin')?'admin':'app');window.addEventListener('hashchange',f);return ()=>window.removeEventListener('hashchange',f)},[]);const [profile,setProfile]=useState(()=>{try{return {...defaults,...JSON.parse(localStorage.getItem('nutrisync-profile'))}}catch{return defaults}}),[auth,setAuth]=useState(()=>{try{return JSON.parse(sessionStorage.getItem('nts-auth'))}catch{return null}}),[loading,setLoading]=useState(false);if(route==='admin')return <AdminApp/>;if(!auth)return <AuthPage onAuth={a=>{setAuth(a);setProfile(p=>({...p,name:a.user.name}))}}/>;const content={overview:<OverviewEnhanced profile={profile} setPage={setPage} />,progress:<Progress profile={profile}/>,water:<Water profile={profile}/>,scan:<ScannerPanel />,reports:<ReportsEnhanced />,calculator:<><BMIGauge profile={profile}/><NutritionCalculator profile={profile}/></>}[page];return <div className={dark?'app dark':'app'}><div style={{position:'fixed',bottom:24,left:'50%',transform:'translateX(-50%)',zIndex:999}}><MacOSDock apps={[
