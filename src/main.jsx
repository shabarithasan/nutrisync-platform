...
function App(){const defaults={name:'',age:'',gender:'',height:'',weight:'',targetWeight:'',activity:'',health:'Normal',healthDetails:'',allergies:'',avoid:'',goal:'loss',diet:'',mealsPerDay:'',cuisine:'',plan:'monthly'};const [page,setPage]=useState('overview'),[dark,setDark]=useState(false),[route,setRoute]=useState(()=>location.hash.startsWith('#admin')?'admin':'app');useEffect(()=>{const f=()=>setRoute(location.hash.startsWith('#admin')?'admin':'app');window.addEventListener('hashchange',f);return ()=>window.removeEventListener('hashchange',f)},[]);const [profile,setProfile]=useState(()=>{try{return {...defaults,...JSON.parse(localStorage.getItem('nutrisync-profile'))}}catch{return defaults}}),[auth,setAuth]=useState(()=>{try{return JSON.parse(sessionStorage.getItem('nts-auth'))}catch{return null}}),[loading,setLoading]=useState(false);if(route==='admin')return <AdminApp/>;if(!auth)return <AuthPage onAuth={a=>{setAuth(a);setProfile(p=>({...p,name:a.user.name}))}}/>;const content={overview:<OverviewEnhanced profile={profile} today={m[todayKey()]||{}} m={m} upd={upd} todayKey={todayKey} setPage={setPage} />,progress:<Progress profile={profile}/>,water:<Water profile={profile}/>,scan:<FoodScan profile={profile}/>,reports:<ReportsEnhanced />,calculator:<><BMIGauge profile={profile}/><NutritionCalculator profile={profile}/></>}[page];return <div className={dark?'app dark':'app'}><div style={{position:'fixed',bottom:24,left:'50%',transform:'translateX(-50%)',zIndex:999}}><MacOSDock apps={[
